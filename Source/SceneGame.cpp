...
void SceneGame::UpdatePlayerSpawn()
{
	...
	{
		if (players.size() < 5) // ※Player.cpp側で s_spawnCount を使う修正をしている場合はそちらに従ってください
		{
			// ★追加: シーンによって禁止範囲を変える
			float limitRange = 30.0f; // デフォルト (scene_tutorial など)

			if (scene_name == "scene_play")
			{
				limitRange = 60.0f; // scene_play の時は範囲を広げる (壁の位置に合わせて調整してください)
			}
