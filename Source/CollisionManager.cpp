	...
	for (size_t i = 0; i < objects.size(); i++)
	{
		for (size_t j = i + 1; j < objects.size(); j++)
		{
				...
				objectB->collider->type == ColliderType::Sphere)
			{
				...
				DirectX::XMFLOAT3 contactPoint; // (IntersectSphereVsCylinder の最後の引数)

				// Collision::IntersectSphereVsCylinder の引数を入れ替えて呼ぶ
				isCollisionDetected =
					Collision::IntersectSphereVsCylinder(
						sphereB->center, sphereB->radius,
				...
				objectB->collider->type == ColliderType::Sphere)
			{
				...
				// Collision::IntersectSphereVsOBB の引数を入れ替えて呼ぶ
				isCollisionDetected = Collision::IntersectSphereVsOBB(
